...
class ExercisesManager: ObservableObject {
    ...
    @Published var exercisesList = Exercise.exercises
    
    func previous(for exercise: Exercise) -> Exercise? {
        if let index = exercisesList.firstIndex(where: {$0.title == exercise.title}) {
            if index > 0 {
                return exercisesList[index - 1]
            }
        }
        return nil
    }
    ...
    func next(for exercise: Exercise) -> Exercise? {
        if let index = exercisesList.firstIndex(where: {$0.title == exercise.title}) {
            if index < exercisesList.count - 1  {
                return exercisesList[index + 1]
            }
        }
        return nil
